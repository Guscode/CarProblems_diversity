...
```{r}
p_load(brms,rethinking)

total$kaiscore <- as.numeric(total$kaiscore)
total$scaled_scores <- scale(total$AllScores)

model <- bf(
  scaled_scores| trunc(lb = -8,ub=3)~kaiscore+aiRange+kaiscore:aiRange, family = skew_normal()
)
...
dens(total$kaiscore)
priors <- c(
  prior(normal(-2.5,.5), class = alpha),
  prior(normal(0, .05), class =Intercept),
  prior(normal(0,.01), class = b),
  prior(normal(0,.02), class = sigma)
)
...
f<- fitted(pp, summary = F, scale = "linear") %>% 
  as_tibble() %>% 
  as.data.frame()
f <- t(f)
f <- as.data.frame(f)
summary(pp)
p <- posterior_samples(pp)
dens(p$lp__)
